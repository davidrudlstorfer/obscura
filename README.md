...
With an NVIDIA GPU:
```
docker run --rm --gpus all -v "<PROJECT_PATH>\render:/workspace/runtime" blender-render-image /workspace/runtime/configs/params.yaml
```
...
```
singularity run --nv -B "<PROJECT_PATH>/render:/workspace/runtime" obscura.sif /workspace/runtime/configs/params.yaml
```
