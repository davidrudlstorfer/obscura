...
bbox = [mathutils.Vector(corner) for corner in mesh_obj.bound_box]
...
cam_location = center + mathutils.Vector((0, -max_dim * 2, max_dim))
...
def setup_lighting(center: mathutils.Vector, max_dim: float, params: dict) -> tuple:
    """Set up three-point lighting using SUN lights."""

    # Key light
    bpy.ops.object.light_add(
        type="SUN", location=center + mathutils.Vector((max_dim, -max_dim, max_dim))
    )
    ...
    # Back light
    bpy.ops.object.light_add(
        type="SUN", location=center + mathutils.Vector((0, 0, max_dim * 1.5))
    )
...
if bg_node:
    bg_node.inputs["Color"].default_value = params.get("background_color", [1, 1, 1, 1])
    bg_node.inputs["Strength"].default_value = params.get("ambient_light_strength", 0.2)
